...
def load_data(opt):
    ...
    src_dir = "i2b2_testsamples_20200330"
    fname = opt.abbr + "_i2b2.txt"
    ...
    # ############# 2020-03-29 ############
    #for exp in sorted(casi_test): 
     #   num_samples = min(100, len(casi_test[exp]))
      #  curr_exp = shuffle(list(sorted(casi_test[exp])), random_state=SEED)[:num_samples]
        ######################################
        #num_samples = min(5, len(casi_test[exp]))
        #curr_exp = shuffle(list(casi_test[exp]), random_state=SEED)[:num_samples]
       # casi_test_list.extend(curr_exp)
    #casi_test = shuffle(casi_test_list, random_state=19)
    #for line in casi_test_list:
     #   print(line)
    #import sys; sys.exit(0)
    exp2id, id2exp = make_id_map(casi_keys)
    #return casi_test, exp2id, id2exp
...
def main():
    ...
    # args.fasttext = "../word_embeddings.bin"
    word_model = fastText.load_model(args.fasttext)

    args.datafile = "i2b2_mimic_rs_dataset_20200307/" + args.abbr + "_rs_close_umls_terms_20200307.txt"
    print(args)
    ...
    print("best epoch/val_loss: {}/{}".format(model.best_epoch, model.best_val_loss))
